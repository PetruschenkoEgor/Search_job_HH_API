from src.operations_with_vacancies import (cast_hh_to_object_list, get_top_vacancies, get_vacancies_by_salary,
                                           get_vacancy_words, print_vacancies, sort_vacancies)
...
def test_cast_hh_to_object_list(get_json_vacancy):
    ...
    assert isinstance(lst_vacancy, list)
    for vacancy in lst_vacancy:
        ...
        assert (
            vacancy.description
            == "Исправление существующих и возникающих багов. Взаимодействие с командой по проекту. Работа с системой "
               "контроля версий Git. Разработка и тестирование backend..."
        )
        assert (
            vacancy.requirements
            == "Уверенное владение Python. Умение писать SQL запросы. Аналитический склад ума. Навык работы с "
               "технической документацией и требованиями. Умение разбираться в..."
        )
...
def test_print_vacancies(capsys, vacancy):
    ...
    assert (
        message.out.strip()
        == "Название вакансии: Разработчик. Город: Москва. Ссылка на вакансию: http:...r. Краткое описание вакансии: "
           "Описание. Требования к кандидату: Требования. Заработная плата: 10000 - 20000."
    )
